...
class user_stat {
    ...
    public function get_user_statement(){
        $this->get_pers_info();
        $akk=$this->person_info->accounts[0]->id;  //id аккаунта
        //

        $this->user_statement=json_decode(file_get_contents($this->api.$this->state.$akk."/".(time()-2682000)."/".time(), false, $this->context));
    ...
    public function db_save_new_transaction(){
        ...
        if($new_transaction != 0) {
            ...
            $result="No new transaction :  ".date("d.m - H:i", time()+3600*3);
        }
        return $result;
